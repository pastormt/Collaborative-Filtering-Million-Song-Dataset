...
def calcSongsInCommon( listenerA, listenerB, songsListeners, listenersSongs, songsSimilarities ):
    inCommon = 0.0
    for songA in listenersSongs[listenerA].iterkeys():
        for songB in listenersSongs[listenerB].iterkeys():
            inCommon += len({k for k in songsListeners[songA] if k in songsListeners[songB]}) / (len(songsListeners[songA]) + len(songsListeners[songB]))
    return inCommon
...
def calcSimsAndRecommend( testListeners, listenersTotalPlays, listenersSongs, songsListeners, pruning, power=1 ):
    """
        ...
        for listenerB, songsDictB in listenersSongs.iteritems(): 
            ...
            if listenerA != listenerB:
                ...
                if len(intersection) != 0:
                    ...
                    # now 3 different measures
                    inCommon.append(calcSongsInCommon(listenerA, listenerB, songsListeners, listenersSongs, songsSimilarities))                 
                    
                    listenerSimilarities[listenerB] = inCommon
        
        # now, for each similarity list (for each listenerB similar to listenerA), normalize
        scaleCoeff = []
        for i in range(0,3): # lenth of inCommon is 3
